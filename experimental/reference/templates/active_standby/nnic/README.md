...
This solution uses a Heat Orchestration Template to launch two nNIC (multi NIC) deployments of a BIG-IP VE in an Openstack Private Cloud. The BIG-IPs will be placed in a sync-only or sync-failover high availablity device service cluster. In a nNIC implementation, one interface is for management and data-plane traffic from the Internet, and the user-provided NIC count determines the number of additional NICs. The first non-management network interface defined is the one that will be used for configuration sychronization and cluster state updates.

The **active_standby** heat orchestration template incorporates existing networks defined in Neutron. 
...
| bigip_cluster_name | No | The Devie Group name to use for this cluster | |
| bigip_cluster_sync_type | No | The type of device group to create. The default is sync-failover | can be 'sync-failover' or 'sync' |
| bigip_cluster_auto_sync | No | Boolean which defines if auto syncrhonization is enabled for the device group | True or False |
| bigip_cluster_save_on_auto_sync | No | Boolean which defines if the BIG-IP configuration will be serialized to configuratoin files when automatic sychronization is triggered | True or False |
